   ...
   def getDistance(self):
      if self.distance == 0:
         ...
         for cityIndex in range(0, self.tourSize()):
            ...
            tourDistance += distMatrix[fromCity.getID()][destinationCity.getID()]
...
class GA:
   ...
   def crossoverFast(self, parent1, parent2):
      parent1Half = parent1.getTour()
      parent2Half = parent2.getTour()
      return parent1Half[:len(parent1)//2] + parent2Half[len(parent1)//2:]
...
if __name__ == '__main__':
   ...
   with open(city_name, 'r') as f:
       for line in f:
           word = line.split(" ")
           new_city = City(int(word[0]), float(word[1]),float(word[2]))
           x1.append(float(word[1]))
           y1.append(float(word[2]))
           tourmanager.addCity(new_city)
           newRow.append(int(word[0]))
   distMatrix.append(newRow)
   
   # """NEW""" - pre-calculates the distance into a distance matrix        
   for i in range (tourmanager.numberOfCities()):
       newRow = [tourmanager[i].getID()]
       for j in range (tourmanager.numberOfCities()):
           if (i == j):
               newRow.append(0)
           elif (j < i and i > 0):
               newRow.append(distMatrix[j][i])
           else:
               newRow.append(tourmanager[i].distanceTo(tourmanager[j]))
       distMatrix.append(newRow)
